 ...
 * Cameras Controller
 */
app.controller('CamerasController', ['$scope', 'MockBackendService', '$document', '$timeout', function ($scope, MockBackendService, $document, $timeout) {
    ...
    $scope.startScanning = function () {
        ...
        // Simulate Scanning Progress without finding fake devices
        var interval = $timeout(function nextStep() {
            $scope.scanProgress += 10;

            if ($scope.scanProgress < 100) {
                $timeout(nextStep, 200);
            } else {
                $scope.isScanning = false;
            }
        }, 200);
